...
@interface MDChineseSort()
//用将需要排序的对象封装在MDChineseSort对象中，包含排序的字符串，对象，首字母三个属性
//进行比较的字符串，
...
+(NSMutableArray*)IndexArray:(NSArray*)stringArr{
    ...
    {
        NSString *pinyin = [((MDChineseSort*)object).pinYin substringToIndex:1];
        //不同
...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr{
    ...
    for (NSString* object in tempArray) {
        ...
        NSString *pinyin = [((MDChineseSort*)object).pinYin substringToIndex:1];
        NSString *string = ((MDChineseSort*)object).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr{
    ...
    {
        MDChineseSort *chineseString = [[MDChineseSort alloc]init];
        ...
        chineseString.string = [MDChineseSort RemoveSpecialCharacter:chineseString.string];
...
+(NSMutableArray*)sortObjectArray:(NSArray*)objectArray Key:(NSString *)key{
    ...
    NSString *tempString;
    //拼音分组
    for (MDChineseSort* object in tempArray) {
    ...
    if (BMLog==1) {
        ...
        NSLog(@"MDChineseSort user time in %f ms", useTime *1000.0);
    }
